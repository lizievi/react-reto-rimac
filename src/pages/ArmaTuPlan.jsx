...
const ArmaTuPlan = () => {
  ...
  let formatNumber = useFormatCurrency()

  return (
  <div>
    <Header className='!bg-white border-b-2 border-[var(--color-borde)]'/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_4.4fr_3.1fr]">
        ...
        <section className="pt-20 flex flex-col gap-4">
          ...
          <div>
            <div className="flex lg:justify-between lg:items-center lg:border-b border-[var(--color-borde)] pt-[30px] pb-[30px] w-[70%]  mx-auto flex-col lg:flex-row gap-5 lg:gap-0">
              ...
              <div className="flex justify-between items-center lg:w-[40%] border border-[var(--color-borde)] rounded-[8px] p-[16px]">
                <button onClick={MountDecrease} className="text-[var(--color-tertiary-light)] text-4xl w-8">-</button>
                <span>{formatNumber(mount)}</span>
                <button onClick={MountIncrease} className="text-[var(--color-tertiary-light)] text-2xl w-8">+</button>
              </div>
            </div>
          </div>

          <h2 className="w-[70%] mx-auto text-[20px] text-[var(--color-secondary-dark)] ">Agrega o quita coberturas</h2>
          <div>
          ...
          <Coberturas className='w-[70%] mx-auto' />
        </section>
        ...
        <section className="lg:pt-48">
          ...
          <div className="flex w-[70%] mx-auto  py-4 items-center lg:flex-col">
            <div className="text-[var(--color-secondary-dark)] lg:pb-4 lg:border-b border-[var(--color-borde)] w-[70%] text-[12px]">
              ...
              <p className="text-[24px] lg:text-[28px]">$35.00</p>
              <p className="text-[var(--color-secondary)] hidden lg:block">mensuales</p>
              <p className="text-[var(--color-secondary)] text-[10px] lg:hidden">MENSUAL</p>
            </div>
            
            <ul className="hidden lg:flex flex-col gap-3 pt-6 pb-10 w-[70%]">
              <p className="text-[var(--color-secondary-dark)]">El precio incluye</p>
              <li className="flex items-center gap-4 text-[14px] text-[var(--color-secondary)]"><FaCheck className="text-[var(--color-check)] text-[12px]" />Llanta de repuesto</li>
              <li className="flex items-center gap-4 text-[14px] text-[var(--color-secondary)]"><FaCheck className="text-[var(--color-check)] text-[12px]"/>Analisis de motor</li>
              <li className="flex items-center gap-4 text-[14px] text-[var(--color-secondary)]"><FaCheck className="text-[var(--color-check)] text-[12px]"/>Aros gratis</li>
            </ul>

            <ButtonPrimary value='LO QUIERO' className='w-[70%] py-0 lg:py-3 lowercase lg:uppercase'/>
          </div>
        </section>
      </div>
